...
@Templated
public class LocalTimePicker implements TakesValue<LocalTime>,
                                        HasValueChangeHandlers<LocalTime> {
    ...
    private Collection<ValueChangeHandler<LocalTime>> valueChangeHandlers;

    private BootstrapDateTimePicker datePicker;

    private LocalTime currentValue;

    private static final String DATE_FORMAT_STRING = "LT";
    ...
    public void init() {
        ...
        datePicker.on("dp.change", () -> {
            ...
            if (value != null && !value.isEmpty()) {
                setValue(LocalTime.parse(MomentJs.moment(value, DATE_FORMAT_STRING).format("HH:mm:ss.SSSSSSSSS")));
            } else {
                setValue(null);
            }
        ...
        if (currentValue == null || !currentValue.equals(value)) {
            ...
            if (value != null) {
                ((BootstrapDateTimePicker.BootstrapDateTimePickerData) datePicker.data("DateTimePicker"))
                        .date(MomentJs.moment(value.toString(), "HH:mm")
                                      .toDate());
            } else {
                ((BootstrapDateTimePicker.BootstrapDateTimePickerData) datePicker.data("DateTimePicker"))
                        .date((String) null);
    ...
    public void fireEvent(GwtEvent<?> event) {
        if (event.getAssociatedType().equals(ValueChangeEvent.getType())) {
            valueChangeHandlers.forEach(h -> h.onValueChange((ValueChangeEvent<LocalTime>) event));
        }
    }

    @Override
    public HandlerRegistration addValueChangeHandler(ValueChangeHandler<LocalTime> handler) {
