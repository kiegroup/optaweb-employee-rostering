...
export const getShiftById = (state: AppState, id: number): Shift => {
  ...
  const view = state.shiftList.shiftMapById.get(id) as DomainObjectView<Shift>;
  return {
    ...view,
    spotId: spotSelectors.getSpotById(state, view.spotId),
    rotationEmployeeId: (view.rotationEmployeeId !== null)?
      employeeSelectors.getEmployeeById(state, view.rotationEmployeeId) : null,
    employeeId: (view.employeeId !== null)?
