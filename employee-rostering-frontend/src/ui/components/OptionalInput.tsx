...
interface OptionalInputProps<T extends Object> {
    defaultValue: T|null;
...
interface OptionalInputState<T extends Object> {
    inputValue: T|null;
    currentValue: T|null;
    isChecked: boolean;
}

export default class OptionalInput<T  extends Object> extends React.Component<OptionalInputProps<T>,OptionalInputState<T>> {
  ...
  handleToggle(isChecked: boolean) {
      if (isChecked) {
          this.setState(prevState => ({currentValue: prevState.inputValue, isChecked: true}));
  ...
  render() {
    ...
    return (
      <span>
        <TextInput isDisabled={!isChecked}
