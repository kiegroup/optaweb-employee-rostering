...
public class Infinite60MinutesScaleTest {
    ...
    public void test() {
        ...
        // Boundaries
        Assert.assertEquals((Double) startInGridPixels, scale.toGridPixels(start), ACCEPTED_ERROR);
        Assert.assertEquals((Double) endInGridPixels, scale.toGridPixels(end), ACCEPTED_ERROR);
        Assert.assertEquals(start, scale.toScaleUnits(startInGridPixels));
        Assert.assertEquals(end, scale.toScaleUnits(endInGridPixels));

        // Over-boundaries
        Assert.assertEquals((Double) (-1d), scale.toGridPixels(start.minus(60, MINUTES)), ACCEPTED_ERROR);
        Assert.assertEquals((Double) (endInGridPixels + 1d), scale.toGridPixels(end.plus(60, MINUTES)), ACCEPTED_ERROR);
        ...
        Assert.assertEquals((Double) (24d * 3 + 4d), scale.toGridPixels(d), ACCEPTED_ERROR);
